...
- 커버는 `assets/covers/`의 검수된 20개 지역 사진을 사용한다. 목록과 상세 모두 16:9 영역에 사진 전체를 표시하고 지명·상태는 아래에 배치한다.
- 목적지는 `lib/models/destination_cover.dart`의 정확한 별칭으로만 연결한다. 미등록·복수 목적지와 로딩 실패는 등고선 그라데이션으로 표시하며 외부 사진 검색을 하지 않는다.
- 사진의 정보 버튼에서 저작자·원본·라이선스를 확인한다. 사진 추가 시 `assets/covers/CREDITS.md`에 출처와 라이선스를 기록하고, 가로형 사진을 실제 카드 크기로 검수한다.
- 오프라인에서도 사진이 표시되는지 확인한다. 추가 이미지 API 키나 서버 설정은 필요 없다.
